...
export default function RightContent() {
    ...
    const onSubmit = async (data) => {
        const { Password } = data; //Du lieu tra ve
        console.log(Password);
        try {
            const token = ParamServices.getResetPasswordToken();
            setLoading(true);
            const responseData = await AuthServices.setNewPassword(
                token,
                Password
            );
            setLoading(false);
        ...
        <div className="box-border h-full w-full bg-white relative px-6 md:px-[100px] pt-10 md:pt-[72px]">
            ...
            <div className="font-[Montserrat] leading-tight w-full">
                ...
                <p className="text-(--color-text-desc) font-medium">
                    Welcome back! Please set the new password
                </p>
            </div>
